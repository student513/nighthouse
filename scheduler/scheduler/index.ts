...
agenda.on("ready", async () => {
  ...
  agenda.define(agendaJobName.GET_ANALYSIS, async () => {
    ...
    for (let url of urls) {
      ...
      await exec(
        `lhci collect --url="${url.url}" --numberOfRuns=5 --settings.chromeFlags="--no-sandbox" --settings.formFactor="desktop" --settings.locale="ko-KR" && lhci upload --target="temporary-public-storage" && lhci upload --target="filesystem" --reportFilenamePattern=%%DATETIME%%-${url._id}.%%EXTENSION%% --outputDir=./reports && rm -rf ./.lighthouseci/lhr-*`
      )
        .then((message) => {
